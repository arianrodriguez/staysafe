...
public:
	...
	void jugar(Graphics^ gr, Bitmap^ imgJugador, Bitmap^ imgVacuna, Bitmap^ imgInfectado1,
		...
		Bitmap^ bmp_ambu, Bitmap^ imgPildora, Bitmap^, Bitmap^, Bitmap^, Bitmap^, Bitmap^, Bitmap^, Bitmap^);
...
Controlador::Controlador(Graphics^ g)
{
	...
	this->tempPildoras = 70;	// aqui controlas el tiempo de en que desaparecen las pildoras una vez generadas (lo cambie a 7s) 
...
void Controlador::agregarBotiquin()
{
	arrBot->push_back(new Botiquin());
}

void Controlador::agregarPildora()
{
	arrPil->push_back(new Pildora());
}

void Controlador::desaparecerPildora()
{
	if (tempPildoras == 0) {
		...
		tempPildoras = 70;	// aqui tambien se cambio con lo de arriba dx
	}
	else {
		tempPildoras--;
	}
...
void Controlador::tiempoInmunidad()
{
	if (temp == 0) {
...
void Controlador::agregarNPC()
{
	arrNPC->push_back(new NPC());

}

void Controlador::agregarDosisNPC(int x, int y) {
	dosis->push_back(new GlobosTexto(x, y));
}

void Controlador::jugar(Graphics^ gr, Bitmap^ imgJugador, Bitmap^ imgVacuna, Bitmap^ imgInfectado1,
	Bitmap^ imgInfectado2, Bitmap^ imgSano1, Bitmap^ imgSano2, Bitmap^ imgBotiquin, Bitmap^ imgVidas, Bitmap^ imgBarraBalas,
	Bitmap^ bmp_ambu, Bitmap^ imgPildora, Bitmap^ imgPersonajeInmune, Bitmap^ txt_soyinmune, Bitmap^ txt_recargando,
	Bitmap^ dosis1, Bitmap^ dosis2, Bitmap^ dosis3, Bitmap^ sindosis)
{
	moverTodo(gr);
	mostrarTodo(gr, imgJugador, imgVacuna, imgInfectado1, imgInfectado2, imgSano1, imgSano2, imgBotiquin, 
		...
		dosis1, dosis2, dosis3, sindosis);
}

void Controlador::moverTodo(Graphics^ gr)
{
...
void Controlador::mostrarTodo(Graphics^ gr, Bitmap^ imgJugador, Bitmap^ imgVacuna, Bitmap^ imgInfectado1,
	Bitmap^ imgInfectado2, Bitmap^ imgSano1, Bitmap^ imgSano2, Bitmap^ imgBotiquin, Bitmap^ imgVidas, Bitmap^ imgBarraBalas,
	Bitmap^ bmp_ambu, Bitmap^ imgPildora, Bitmap^ imgPersonajeInmune, Bitmap^ txt_soyinmune, Bitmap^ txt_recargando,
	Bitmap^ dosis1, Bitmap^ dosis2, Bitmap^ dosis3, Bitmap^ sindosis)
{
...
void Controlador::colision(vector<Rectangle*>* paredes)
{
	...
	for (int i = 0; i < arrV->size(); i++)
	{
		for (int j = 0; j < arrNPC->size(); j++)
		{
			if (arrV->at(i)->colision(arrNPC->at(j)->getRectangle()) && arrNPC->at(j)->getEstado() == 'E') {
				...
				/*
				dosis->at(aux_dosis)->setindnpc(j); //para saber a que NPC le corresponde tal bitmap de dosis
				aux_dosis++;
				*/
				if (arrNPC->at(j)->getCantDosis() == 2) {
				...
				//arrNPC->at(j)->setVisibilidad(false);	// esto es temporal
			}
		}
	}
...
void Controlador::refill_balas(Graphics^ g) {
...
void Controlador::actualizar_seg() {
	if (this->cont_ax > 0) {
		this->cont_seg--;
		if (this->cont_seg == 0) {
...
bool Controlador::time_over() { return(this->cont_ax == 0) ? true : false; }
